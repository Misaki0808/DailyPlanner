...
describe('AI yanıt işleme', () => {
  ...
  describe('extractTimesFromTextWithAI', () => {
    it('markdown çitli JSON’u ayrıştırır', async () => {
      fetchMock.mockResolvedValue(
        makeGeminiResponse('```json\n[{"hour":8,"minute":30,"label":"Kalk"}]\n```')
      );
      const { extractTimesFromTextWithAI } = require('../../src/utils/aiService');

      await expect(extractTimesFromTextWithAI("sabah 8:30'da kalk")).resolves.toEqual([
        { hour: 8, minute: 30, label: 'Kalk' },
      ]);
    });

    // Yalnız tip kontrolü yapılsaydı hour=99 geçer ve setHours(99, ...) alarmı
    // günlerce ileri kaydırırdı.
    it('aralık dışı saat/dakika değerlerini eler', async () => {
      fetchMock.mockResolvedValue(
        makeGeminiResponse(
          '[{"hour":99,"minute":0,"label":"Bozuk"},' +
          '{"hour":8,"minute":200,"label":"Bozuk2"},' +
          '{"hour":-1,"minute":0,"label":"Bozuk3"},' +
          '{"hour":9,"minute":15,"label":"Geçerli"}]'
        )
      );
      const { extractTimesFromTextWithAI } = require('../../src/utils/aiService');

      await expect(extractTimesFromTextWithAI('metin')).resolves.toEqual([
        { hour: 9, minute: 15, label: 'Geçerli' },
      ]);
    });

    it('boş etiketli maddeyi eler', async () => {
      fetchMock.mockResolvedValue(makeGeminiResponse('[{"hour":9,"minute":0,"label":"  "}]'));
      const { extractTimesFromTextWithAI } = require('../../src/utils/aiService');

      await expect(extractTimesFromTextWithAI('metin')).resolves.toEqual([]);
    });

    it('model dizi döndürmezse yerel ayrıştırmaya düşer', async () => {
      fetchMock.mockResolvedValue(makeGeminiResponse('{"hour":9}'));
      const { extractTimesFromTextWithAI } = require('../../src/utils/aiService');

      await expect(extractTimesFromTextWithAI("sabah 8'de kalk")).resolves.toEqual([
        { hour: 8, minute: 0, label: 'Kalk' },
      ]);
    });
  });
});
